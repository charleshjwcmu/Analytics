...
#    date_list = formatDate(dates[table_name])
    ...
    if table_name == "PBUP_OPSDEP_FUNDBAL_OUTPUT_UP":
        ...
        "PRIN_PARENT_NODE","PRIN_PARENT_NODE_DESC","PRINCIPAL_BAL_USD","PRINCIPAL_BAL",\
        ...
        "SOURCE_PRODUCT_TYPE","TRANSACTION_CURRENCY","FX_BASE_TO_USD","IFS_GNA_PRIN","SITE"]
#        "INTEREST_RATE","SITE","SOURCE_SYSTEM_ID","PERIOD_ID","BRANCH","CDMS_COUNTERPARTY_ID","CDMS_COUNTERPARTY_NAME",
        ...
        data_all = read_database(file_list,required_attributes)
                        ...
                        "PBUP_OPSDEP_FUNDBAL_OUTPUT_UP_GTRM","OPS_DEPOSIT_PBUP_FBO_DDA_GTRM","OPS_DEPOSIT_DDA_RAI_GTRM","OPS_DEPOSIT_UP_RAI_GTRM","PBUP_OPSDEP_RAI_GTRM_AltCluster",\
                        "PBUP_OPSDEP_RAI_GTRM_IntradayRatio90","PBUP_OPSDEP_RAI_GTRM_IntradayRatio95","PBUP_OPSDEP_RAI_GTRM_IntradayRatio100","PBUP_OPSDEP_RAI_GTRM_IntradayRatio105","PBUP_OPSDEP_RAI_GTRM_IntradayRatio110",\
                        "FX_RATES",'CORE_BALANCE','CORE_BALANCE_USD','BALANCEBYCURRENCY','BALANCEBYCURRENCY_USD','PBUP_OPSDEP_FUNDBAL_OUTPUT_UP_WITH_TRANSACTION_GTRM']:
        out_directory = PATH_DB+table_name+"/HdfData/"
        file_list = [out_directory + table_name + "_" + date + ".hdf" for date in date_list]
    ...
    "Total_Deposits_By_ProductTypeNew","Operational_Deposits_By_ProductTypeNew",\
    "FX_RATES_Summary","BALANCEBYCURRENCY_Summary","BALANCEBYCURRENCY_USD_Summary",\
    "Payment_By_ParentRegion","Balance_By_ParentRegion",\
    "Outflow_By_Region","Outflow_By_BehaviorGroup"]:
    ...
    elif table_name in ["Total_Deposits_By_LegalEntity_and_Currency",\
    "Total_Deposits_By_LegalEntity_and_BehaviorGroup",\
    "Total_Deposits_By_ProductTypes_and_Currency",\
    "Total_Deposits_By_BehaviorGroup_and_Currency",\
    "Total_Deposits_By_ParentCompy_and_Currency"]:
        out_directory = PATH_DB+"MISC_TABLES/RawData/"
        file = out_directory + table_name + ".xlsx"
        data_all = pd.ExcelFile(file).parse("Sheet1",index_col=[0,1])
        data_all = data_all.transpose().sort_index().transpose()
...
def read_db_test(table_name, date_list=None):
#    date_list = '20170429'
#    date_list = formatDate(dates[table_name])
    if date_list == None:
        base = datetime.datetime.today()
    ...
    if table_name == "TRANS_PROD_TREASURY_COMMON_DEPOSITS":
        out_directory = PATH_DB+table_name+"/HdfData/"
        file_list = [out_directory + table_name + "_" + date + ".hdf" for date in date_list]
        required_attributes = ["AS_OF_DATE","ASSET_LIABILITY_CODE","TDR_STATUS_FLAG",\
        "COUNTRY_DOMICILE","FUND_NUM","INTERNAL_COMPANY_FLAG",\
