...
#    current_diff = summarypage2.diff(-1).head(1).dropna()
    ...
    for i in range(len(attributes)):
#        i = 0
        p = stats.percentileofscore(summmary_result.diff(-1).dropna()[attributes[i]], current_diff[attributes[i]].values[0], kind = 'rank')/100
        p = '{:.0%}'.format(p)  
        percentiles.append(p)
...
today = dates[0]
yesterday = dates[1]
#yesterday = today + dateutil.relativedelta.relativedelta(days=-7)
...
#data prep
pivot_attributes = [column_name_group,column_name_company,column_name_currency,column_name_region,'ProductTypes',column_name_division,column_name_stylegroup]
