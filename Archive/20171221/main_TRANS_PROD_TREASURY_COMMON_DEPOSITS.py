...
data_all[column_name_product_type] = [prod_type_mapping[x] for x in data_all[column_name_product_type]]
...
#summary3
def aggregate_summary_table_with_percentiles(summmary_result, current_date = None, reference_dates = None, attributes=[column_name_spot,column_name_operational,column_name_excess]):
    ...
    if reference_dates is None:
        ...
        print("Wrong input.")
        return(None)
    summarypage1 = round(summarypage1*1e-9,2)
    summarypage1['Average']=round(summmary_result.apply(np.mean,axis=0)*1e-9,2)
    summarypage1['Max']=round(summmary_result.apply(max,axis=0)*1e-9,2)
    summarypage1['Min']=round(summmary_result.apply(min,axis=0)*1e-9,2)
    summarypage1['Std']=round(summmary_result.apply(np.std,axis=0)*1e-9,2)
...
#onemonthago = dates[abs(dates-onemonthago) == min(abs(dates-onemonthago))][0]
#month_dates = [today - datetime.timedelta(days=x) for x in range((today-onemonthago).days+1)]
#data_date = data_all[data_all['AS_OF_DATE'].isin(month_dates)]
#table_stat = summarizeTable(data_date,column_name_parent,column_name_spot,fillna=True)
#table_stat = round(pd.concat([table_stat.apply(np.max,axis=1),table_stat.apply(np.min,axis=1)],axis=1)/1e9,2)
#table_stat.columns = ["Max","Min"]

###Behavior groups    
data_all_tmp = data_all[attribute_aggs +["AS_OF_DATE","ND_IND",column_name_parent,column_name_group]]
...
data_all_tmp = data_all[attribute_aggs+['ProductTypes',"AS_OF_DATE"]]
IBDDADomestic = ['IBDDA-MONEY MARKET','IBDDA-NOW ACCOUNTS','IBDDA-EXTERNAL']
data_all_tmp.loc[[x in IBDDADomestic for x in data_all_tmp[attribute_groupby]],attribute_groupby] = "IBDDA-DOMESTIC"
data_all_tmp = data_all_tmp[data_all_tmp[attribute_groupby].isin(['DDA-DOMESTIC','IBDDA-CAYMAN','IBDDA-DOMESTIC'])]
result_total = aggregate_summary_table_format_combined(data_all_tmp,today,yesterday,attribute_groupby,column_name_spot)
result_total.columns = [today.strftime("%m-%d")+"[Total]",yesterday.strftime("%m-%d")+"[Total]","change"]
result_operational = aggregate_summary_table_format_combined(data_all_tmp,today,yesterday,attribute_groupby,column_name_operational)
result_operational.columns = [today.strftime("%m-%d")+"[Opertnl]",yesterday.strftime("%m-%d")+"[Opertnl]","change"]
...
#            subs.to_latex(plot_folder+"/Trend_"+aggregate_groupby+"_"+fund_list[i]+"_details.tex",index=False,longtable=True)
            ...
            summarypage_spot = aggregate_summary_table_with_percentiles(total,attributes=total.columns)
            ...
            summarypage_ope = aggregate_summary_table_with_percentiles(total,attributes=total.columns)
            ...
            behavoior_color_map = {"INTRA-DAY":"red","Multi-Day High":"blue","Multi-Day Low":"yellow"}
            ...
            if len(ope.columns) > 0:
                ...
                fig, ax = plt.subplots()
#                spot.columns = spot.columns + ":TotalBal"
#                ope.columns = ope.columns + ":OpertnlBal"
...
#                j = 0
#                behaviors
#                ax.fill_between(spot.index, spot.iloc[:,j] - 1e8, spot.iloc[:,j] + 1e8, facecolor='yellow', alpha=0.5,label='1 sigma range')
...
#"\\newcommand\\GroupbyAttributes{"+",".join(attribute_groupbys)+"}\n"+\
#"\\newcommand\\GroupbyAttributesTwo{"+",".join(attribute_groupbysParent)+"}\n"+\
#+\
#"\\newcommand\\StartDate{"+earliestdate.strftime("%Y/%m/%d")+"}\n"+\
#"\\newcommand\\DepositDrop{"+str(billions(TotalWidthdraw))+"}\n"+\
