...
from Functions_Analysis import summarizeTableByAttribute, summarizeTable, process_dda_GTRM #, summarizeTableRatioOfTwoFactor, listisin
...
#    sum(table_all.index=="Total")
    ...
    month_dates = [max(currentsnapshot_tmp) - datetime.timedelta(days=x) for x in range(30)]
...
result_total_lastweek.columns = [today.strftime("%m/%d/%y"),yesterday.strftime("%m/%d/%y"),"change"]
...
result_operational_lastweek.columns = [today.strftime("%m/%d/%y"),yesterday.strftime("%m/%d/%y"),"change"]
