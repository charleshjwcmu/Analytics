...
cleanup_start = datetime.datetime.strptime("2018-01-11",'%Y-%m-%d')
...
def execute_ORM_FBO(dates, Verbose = False):
    ...
    file_name_up_hdf = myuptable_dir+"/HdfData/PBUP_OPSDEP_FUNDBAL_OUTPUT_UP_GTRM_"+formatDate(dates.name)+".hdf"
...
##### Daily Balance data
def update_daily_balance_master_file(file_total_deposit,file_total_deposit_client,date_mapping,balance_type,update_start=update_start,attribute_groupby='ULT_PARENT_CD'):
...
def update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby,update_start=update_start):
