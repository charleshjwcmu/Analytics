...
if UPDATE:
    table_names = ["PBUP_OPSDEP_DDA_XREF","PBUP_OPSDEP_VOL_MITG_SPOT_BAL","PBUP_OPSDEP_PREPROC"]
    for table_name in table_names:
        raw_directory = PATH_DB+table_name+"/RawData"
        out_directory = PATH_DB+table_name+"/HdfData"
...
#    date_list = formatDate(dates[table_name])
    ...
    if table_name == "PBUP_OPSDEP_FUNDBAL_OUTPUT_UP":
        ...
        data_all = read_database(file_list,required_attributes)
    elif table_name == "OPS_DEPOSIT_PBUP_FBO_DDA_GTRM":
        # table OPS_DEPOSIT_PBUP_FBO_DDA_VW
        out_directory = PATH_DB+table_name+"/HdfData/"
        file_list = [out_directory + table_name + "_" + date + ".hdf" for date in date_list]
        ...
        "IFS_PRODUCT","IFS_COMPANY","PROG_CONSOL_RESP_CENTER_ID","PRODUCT_SUB_TYPE_DESC","PRODUCT_TYPE_DESC","SOURCE_CONTRACT_NUM",\
...
#        "INTEREST_RATE","SITE","SOURCE_SYSTEM_ID","PERIOD_ID","BRANCH","CDMS_COUNTERPARTY_ID","CDMS_COUNTERPARTY_NAME",
        ...
        data_all = read_database(file_list,required_attributes)
    elif table_name in ["REF_CDMS_CUSTOMER", "OPSDEP_MASTERATTR","PBUP_OPSDEP_VOL_MITG_SPOT_BAL","OPSDEP_PREPROC","PBUP_OPSDEP_PREPROC","PBUP_OPSDEP_FUNDBAL_OUTPUT_UP_GTRM"]:
        out_directory = PATH_DB+table_name+"/HdfData/"
        file_list = [out_directory + table_name + "_" + date + ".hdf" for date in date_list]
        data_all = read_database(file_list)
    ...
    "Total_Deposits_By_LegelEntity","Operational_Deposits_By_LegelEntity",\
