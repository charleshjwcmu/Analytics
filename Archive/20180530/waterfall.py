...
#            sumvalues[i] = sumvalues[i - 1]
            blanks[i] = sumvalues[i] + values[i]
            neg_h[i] = sumvalues[i-1]
            negblanks[i] = sumvalues[i]

    xpos = [bar_width * x for x in range(len(values))]
    
    fig, ax = plt.subplots()
#    ax.yaxis.set_major_formatter(formatter_billionsinteger)

    ylim(ymin=min(sumvalues)*0.99,ymax=max(sumvalues)*1.01) 
