...
from Functions_Analysis import summarizeTableByAttribute, summarizeTable, process_dda_GTRM, formatDate, render_mpl_table #, summarizeTableRatioOfTwoFactor, listisin
...
formatter_percentages = FuncFormatter(percentages)
...
def pivot_tables(data_all_tmp, index, pivot_attributes, today=today,yesterday=yesterday,func = np.nansum, color_function=None, output_excel = True, output_tex = True, output_png = True):
...
#            render_mpl_table(tmp_results, col_width=2.0)
            fig = render_mpl_table(tmp_results, data_color = pivot_diff, func=color_function, col_width=2.0,row_height=0.4)
            if output_png:
        ...
        pivot_today.ix['Sum']=pivot_today.apply(np.nansum,axis=0)
...
plt.figure()
tmp = result_ope.ix[today,labels]
...
fx_summary = fx_summary.ix[balance_by_currency.index]
...
#    attribute = column_name_product_typenew
    balance_type = column_name_spot
...
#            subs.to_latex(plot_folder+"/Trend_"+aggregate_groupby+"_"+fund_list[i]+"_details.tex",index=False,longtable=True)
            ...
            fig, ax = plt.subplots()
            (payments/spot).dropna().rolling(21*3).apply(np.nanmean).dropna().plot(ax=ax, style=styles)
            ax.legend(loc='best')
            ax.tick_params(labelsize=20)
            ax.xaxis.label.set_visible(False)
