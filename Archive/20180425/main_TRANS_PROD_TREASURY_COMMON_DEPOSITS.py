...
from Functions_Analysis import summarizeTableByAttribute, summarizeTable, process_dda_GTRM, formatDate #, summarizeTableRatioOfTwoFactor, listisin
...
#        summarypage1['DailyChg']=summarypage1.iloc[:,0]-summarypage1.iloc[:,1]
        ...
        current_date = summarypage1.ix[[today],:]
        summarypage1 = summarypage1.ix[[today,yesterday],:].transpose()
        summarypage1.columns = [x.strftime("%Y-%m-%d") for x in summarypage1.columns]
...
#    summmary_result=result_total
    ...
    daycount = np.busday_count(today, yesterday)
    
    current_diff = summarypage2.ix[[today,yesterday]].diff(-1).dropna()
...
if TEST_ENVIRONMENT:
#    end_date = datetime.datetime.strptime('2017-12-29 00:00:00', '%Y-%m-%d %H:%M:%S')     # historical data end date - later than benchmark date
...
#    else:
    ...
    if len(result_history.columns)!=len(result_total.columns):
        ...
        if not (abs(result_history.ix[result_total.index]-result_total)<1e6).all().all():
...
#pivot tables
def pivot_tables(data_all_tmp, index, pivot_attributes, today=today,yesterday=yesterday,func = np.nansum, output_excel = True, output_tex = True):
...
#data prep
pivot_attributes = [column_name_group,column_name_company,column_name_currency,column_name_region,'ProductTypes',column_name_division,column_name_stylegroup,column_name_market]
...
result_total = summarizeTable(data_all_tmp,attribute_groupby,column_name_spot).transpose().sort_index(ascending=False)
result_total = result_total.ix[:,labels]
result_total['Total'] = result_total.apply(np.nansum,axis=1)
df = result_total.ix[today] - result_total.ix[yesterday]
df = df[df.index!="Total"]
df = df[df!=0]
...
result_ope = summarizeTable(data_all_tmp,attribute_groupby,column_name_operational).transpose().sort_index(ascending=False)
result_ope = result_ope.ix[:,labels]
result_ope['Total'] = result_ope.apply(np.nansum,axis=1)
df = result_ope.ix[today] - result_ope.ix[yesterday]
df = df[df.index!="Total"]
df = df[df!=0]
...
# Plot
plt.figure()
plt.pie(result_total.ix[today,labels],autopct='%1.1f%%',explode=explode,shadow=True,colors=piecolors,
        labels = ["Clients 1-10", "Clients 11-50", "Clients 51-100", "Clients 101-200", "Clients 201-500", "Clients 501+","UnregulatedFunds","Funds Miss Model Info"])
plt.axis('equal')
plt.tight_layout()
...
plt.figure()
plt.pie(result_ope.ix[today,labels],autopct='%1.1f%%',explode=explode,shadow=True,colors=piecolors,
        labels = ["Clients 1-10", "Clients 11-50", "Clients 51-100", "Clients 101-200", "Clients 201-500", "Clients 501+","UnregulatedFunds","Funds Miss Model Info"])
plt.axis('equal')
plt.tight_layout()
...
percentiles = aggregate_summary_table_with_percentiles(data_orig_tmp.sort_index(ascending=False))["Percentile"].ix[result_total.index]
...
#    attribute = column_name_currency
    ...
    data_plot.columns=[a_+"("+b_+")" for a_, b_ in zip(data_plot.columns, data_plot.div(data_plot.sum(axis=1),axis=0).mean().map(percentages))]
    plt.figure()
    ...
    if attribute == column_name_currency:
        fig.savefig(weekly_table_dir+"/History_Chart_By_Attribute_"+balance_type+"_"+attribute+".png",bbox_inches='tight')
...
mapping_attribute = read_db(table_name,formatDate(date_mapping.ix[today,table_name]))
...
def graph_fund_list(fund_list):
    ...
    fund_list = fund_list[~pd.isnull(fund_list)]
    fund_list = fund_list[~fund_list.isin(['Fund Miss Model Info','UnregulatedFunds','Other','Others'])]
