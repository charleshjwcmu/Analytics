...
#    update_start = datetime.datetime.strptime("2018-04-01",'%Y-%m-%d')
#    update_end = datetime.datetime.strptime("2018-04-30",'%Y-%m-%d')
    update_end = datetime.datetime.today().strftime('%Y-%m-%d')
    UPDATE_RAI_GTRM = False
    UPDATE_MISC_TABLES = True
...
cleanup_start = datetime.datetime.strptime("2018-12-31",'%Y-%m-%d')
...
#    attributes = [column_name_division,column_name_stylegroup,'COUNTRY_DOMICILE','ProductTypes',column_name_currency,column_name_company,column_name_region,column_name_group,"ID",column_name_parent]
    ...
    for file_name in file_names:
        ...
        data = data.transpose().sort_index().transpose()
       
        select = (data.columns >= cleanup_start)&(data.columns <= cleanup_end)
        data.ix[:,~select].to_excel(file_name)
        
    file_names = []
    file_total_deposit = misc_dir+"/RawData/Total_Deposits_By_LegalEntity_and_BehaviorGroup.xlsx"
    file_total_deposit_client = misc_dir+"/RawData/Operational_Deposits_By_LegalEntity_and_BehaviorGroup.xlsx"
    file_names = file_names + [file_total_deposit,file_total_deposit_client]
    file_total_deposit = misc_dir+"/RawData/Total_Deposits_By_LegalEntity_and_Currency.xlsx"
    file_total_deposit_client = misc_dir+"/RawData/Operational_Deposits_By_LegalEntity_and_Currency.xlsx"
    file_names = file_names + [file_total_deposit,file_total_deposit_client]
    file_total_deposit = misc_dir+"/RawData/Total_Deposits_By_ParentCompy_and_Currency.xlsx"
    file_total_deposit_client = misc_dir+"/RawData/Operational_Deposits_By_ParentCompy_and_Currency.xlsx"
    file_names = file_names + [file_total_deposit,file_total_deposit_client]
    file_total_deposit = misc_dir+"/RawData/Total_Deposits_By_ProductTypes_and_Currency.xlsx"
    file_total_deposit_client = misc_dir+"/RawData/Operational_Deposits_By_ProductTypes_and_Currency.xlsx"
    file_names = file_names + [file_total_deposit,file_total_deposit_client]
    file_total_deposit = misc_dir+"/RawData/Total_Deposits_By_BehaviorGroup_and_Currency.xlsx"
    file_total_deposit_client = misc_dir+"/RawData/Operational_Deposits_By_BehaviorGroup_and_Currency.xlsx"
    file_names = file_names + [file_total_deposit,file_total_deposit_client]
    ...
    for file_name in file_names:
        if not os.path.exists(file_name):
            continue
        print(file_name+": File Updated")
        data = pd.ExcelFile(file_name).parse('Sheet1',index_col=[0,1])
        data = data.transpose().sort_index().transpose()

        select = (data.columns >= cleanup_start)&(data.columns <= cleanup_end)
        data.ix[:,~select].to_excel(file_name,merge_cells=False)
...
#    data_common_clean = data_common
    ...
    # assign transaction/product types
    data_common_clean['ProductTypeNew'] = ""
    data_common_clean.loc[data_common_clean['IFS_GNA_PRIN'].isin([210101140,210101190]),'ProductTypeNew'] = "NIBDDA"
    data_common_clean.loc[data_common_clean['IFS_GNA_PRIN'].isin([220202100]),'ProductTypeNew'] = "IBDDA"
    data_common_clean.loc[data_common_clean['IFS_GNA_PRIN'].isin([220501110]),'ProductTypeNew'] = "Cayman"
    data_common_clean.loc[data_common_clean['IFS_GNA_PRIN'].isin([220501100,220501260,220501261])&data_common_clean['IFS_PRODUCT'].isin([35205])&(~data_common_clean['SITE'].isin(['EEC'])),'ProductTypeNew'] = "Transact"
    data_common_clean.loc[data_common_clean['ProductTypeNew'] == "",'ProductTypeNew'] = "Other"
    data_common_clean.loc[~data_common_clean['TRANSACTION_CURRENCY'].isin(["AUD", "CAD", "CHF",  "EUR", "GBP", "JPY", "USD"]),'TRANSACTION_CURRENCY'] = "OTHER"
    data_common_clean.loc[data_common_clean['ProductTypeNew']=="Transact",'ProductTypeNew'] = data_common_clean.loc[data_common_clean['ProductTypeNew']=="Transact",'ProductTypeNew']+"_"+data_common_clean.loc[data_common_clean['ProductTypeNew']=="Transact",'TRANSACTION_CURRENCY']
...
#        data_common_clean.to_excel(plot_folder+"/data_common_clean_"+formatDate(dates.name)+".xlsx")
    ...
    ## delete deposits from FHLB Atlanta
    source_contract_num = ["507051490","507051594","1493870543","507051638","507051641","1054580679","507051684","507051689"]
    if np.nansum(data_common_clean['SOURCE_CONTRACT_NUM'].isin(source_contract_num)) > 0:
        fhlb_atlanta = millions(data_common_clean.loc[data_common_clean['SOURCE_CONTRACT_NUM'].isin(source_contract_num),"PRINCIPAL_BAL"].apply(np.nansum).values[0])
        print("FHLB Atlanta Deposit: " + fhlb_atlanta + " - to be excluded")
        data_common_clean = data_common_clean[~data_common_clean['SOURCE_CONTRACT_NUM'].isin(source_contract_num)]
        print("Spot Balance (Excluding FHLB Atlanta) is " + str(millions(sum(data_common_clean["PRINCIPAL_BAL_USD"]))))
    else:
        import sys
        sys.exit("Error: FHLV Atlanta deposit is not correctly identified")
#    elif np.nansum(data_common_clean['SRC_COUNTERPARTY_NAME'].isin(["FEDERAL HOME BANK OF ATLANTA"])) > 0:
#        fhlb_atlanta = millions(data_common_clean.loc[data_common_clean['SRC_COUNTERPARTY_NAME'].isin(["FEDERAL HOME BANK OF ATLANTA"]),"PRINCIPAL_BAL"].apply(np.nansum).values[0])
#        print("FHLB Atlanta Deposit: " + fhlb_atlanta + " - to be excluded")
#        data_common_clean = data_common_clean[~data_common_clean['SRC_COUNTERPARTY_NAME'].isin(["FEDERAL HOME BANK OF ATLANTA"])]
#        print("Spot Balance (Excluding FHLB Atlanta) is " + str(millions(sum(data_common_clean["PRINCIPAL_BAL_USD"]))))
...
#for i in range(start,561):
#for i in range(577,end+1):
#    i = 632
...
#        attribute_groupby = [column_name_company,column_name_group]
    ...
    #                table_name = "OPS_DEPOSIT_PBUP_FBO_DDA_GTRM"
                    ...
                    print("Already in "+ date_mapping.index[i].strftime("%Y%m%d"))    
    ...
    ###Two dimentional Pivot Tables
    #legal entity/currency and spot
    balance_type = column_name_spot
    attribute_groupby = [column_name_company,column_name_currency]
    ...
    #legal entity/Behavior Group and spot
    balance_type = column_name_spot
    attribute_groupby = [column_name_company,column_name_group]
    file_total_deposit_client = misc_dir+"/RawData/"+balance_type+"_By_"+ "_and_".join(attribute_groupby)+".xlsx"
    update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby)
    if UPDATE_RAI_GTRM:
        file_total_deposit_client = misc_dir_rai_gtrm+"/RawData/"+balance_type+"_By_"+ attribute_groupby+".xlsx"
        update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby,table_name = table_name_dda_rai_gtrm)
    #Behavior group/currency and spot
    balance_type = column_name_spot
    attribute_groupby = [column_name_group,column_name_currency]
    file_total_deposit_client = misc_dir+"/RawData/"+balance_type+"_By_"+ "_and_".join(attribute_groupby)+".xlsx"
    update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby)
    if UPDATE_RAI_GTRM:
        file_total_deposit_client = misc_dir_rai_gtrm+"/RawData/"+balance_type+"_By_"+ attribute_groupby+".xlsx"
        update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby,table_name = table_name_dda_rai_gtrm)
    #Product type/currency Group and spot
    balance_type = column_name_spot
    attribute_groupby = ['ProductTypes',column_name_currency]
    file_total_deposit_client = misc_dir+"/RawData/"+balance_type+"_By_"+ "_and_".join(attribute_groupby)+".xlsx"
    update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby)
    if UPDATE_RAI_GTRM:
        file_total_deposit_client = misc_dir_rai_gtrm+"/RawData/"+balance_type+"_By_"+ attribute_groupby+".xlsx"
        update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby,table_name = table_name_dda_rai_gtrm)
    #Client/currency Group and spot
    balance_type = column_name_spot
    attribute_groupby = [column_name_parent,column_name_currency]
    file_total_deposit_client = misc_dir+"/RawData/"+balance_type+"_By_"+ "_and_".join(attribute_groupby)+".xlsx"
    update_daily_balance_master_file_from_DDA(file_total_deposit_client,date_mapping,balance_type,attribute_groupby)
    if UPDATE_RAI_GTRM:
        file_total_deposit_client = misc_dir_rai_gtrm+"/RawData/"+balance_type+"_By_"+ attribute_groupby+".xlsx"
